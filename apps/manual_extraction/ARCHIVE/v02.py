    ...
    for i in range(total_rows):
        if not st.session_state["extractions"][i]:  # Only initialize if empty
            ...
            st.session_state["extractions"][i] = {
                "values": default_values.copy(),  # Use default values instead of empty dict
                "row_index": i,
...
def render_nested_field(field_name, field_info, current_value, prefix, depth, base_color):
    ...
    if base_type == int:
        if is_opt:
            ...
            modes = ["(None)", "Number"]
            # Check session state first for mode
...
def generate_export_data():
    ...
    for i, extraction in enumerate(extractions):
        ...
        if extraction:  # Only if extraction exists (not empty dict)
            ...
            # Add index and ID
            # Ensure ID is first key in dict
            id_val = extraction.get("id", f"row_{i+1}")
            new_flat = {"id": id_val, "row_index": extraction.get("row_index", i)}
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state["session_type"] == "Continue Previous":
            ...
            if prev_json is not None:
                ...
                if 'extractions' in file_data.get('data', {}) or 'extractions' in file_data:
                    if restore_previous_export(file_data.get('data', file_data)):
    ...
    # Automatically initialize all rows in memory to ensure complete exports
